...
int tcp_passive_open(tcpsock_t ** sock, int port)
{
  ...
  // return a socket decriptor. return -1 if failure happens
  ...
  //any address on local machine ; internet byte order
  addr.sin_port = htons(port);
  //port number ; host to network short
  ...
  //listen for connections on a socket. Marks it as a possive socket;
  //max_pending defines the maximum length to which the queue of pending connections for sockfd may grow
  ...
  s->port = port;
  s->cookie = MAGIC_COOKIE; //represent the socket is bound
  *sock = s;
  return TCP_NO_ERROR;  
}
...
int tcp_active_open(tcpsock_t ** sock, int remote_port, char * remote_ip)
{
  ...
  //convert char array to 32 byte ip address and save it in addr
  TCP_ERR_HANDLER(result==0,free(client);return TCP_ADDRESS_ERROR);
  addr.sin_port = htons(remote_port);
  //convert it into internet order
  ...
  //getsockname() returns the current address to which the socket is bound, in the buffer pointed to by addr
  //difference between sockaddr and sockaddr_in --- former one mix the port and ip information together but the latter one split them into two parameters
  ...
  //ntohs() convert internet order into integer
...
{  
  ...
  if ((*socket)->cookie == MAGIC_COOKIE) // socket is bound
  {
    ...
    if ((*socket)->sd >= 0) 
    {
      ...
      //destroy the socket so that other process cannot use it either
      //SHUT_RDWR --close read and write function
...
int tcp_wait_for_connection(tcpsock_t * socket, tcpsock_t ** new_socket) 
{
  ...
  //accept a connection on a socket.It is called after listen()
...
{
  //buffer is used to save received data
  TCP_ERR_HANDLER(socket==NULL,return TCP_SOCKET_ERROR);
  TCP_ERR_HANDLER(socket->cookie!=MAGIC_COOKIE,return TCP_SOCKET_ERROR); 
  ...
  *buf_size = recv(socket->sd, buffer, *buf_size, 0);
  ...
  //return the length of the message on successful completion, it might be less than intial one
  ...
  TCP_ERR_HANDLER(*buf_size<0,return TCP_SOCKOP_ERROR); 
  //return value -- <0 error; =0 connection is closed; >0 length of message
  return TCP_NO_ERROR;
}

int tcp_receive_by_sd(int sd, void *buffer, int * buf_size)
{
  TCP_ERR_HANDLER(sd<=0,return TCP_SOCKET_ERROR);
  if ( ( buffer == NULL ) || (buf_size ==0) )  //nothing to read
  {
    *buf_size = 0;
    return TCP_NO_ERROR; 
  }
  *buf_size = recv(sd, buffer, *buf_size, 0);
  TCP_DEBUG_PRINTF(*buf_size==0,"Recv() : no connection to peer\n");
  TCP_ERR_HANDLER(*buf_size==0,return TCP_CONNECTION_CLOSED); 
  TCP_DEBUG_PRINTF((*buf_size<0)&&(errno==ENOTCONN),"Recv() : no connection to peer\n");
  TCP_ERR_HANDLER((*buf_size<0)&&(errno==ENOTCONN),return TCP_CONNECTION_CLOSED);
